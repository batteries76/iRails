...
users.each do |user|
    if user.email == "p@p.com"
        user.update(role: Role.find(2))
    elsif user.email == "m@m.com"
        user.update(role: Role.find(3))
    else 
        user.update(role: Role.find(1))
    end
end 

puts "UPDATED THE ROLES OF USERS"